...
const char *supported_scopes[] = {"server"};
...
int cmd_init(int argc, const char **argv) 
{
    ...
    if (argc < 2)
    {
        fprintf(stderr, "hint: Maybe you wanted to say 'initx init <language> <project_name> [scope]'?\n");
        return 1;
    }

    const char *language = argv[0];
    const char *project_name = argv[1];
    const char *scope = (argc >= 3) ? argv[2] : "console";
    ...
    base_template_struct base_template = 
    {
        .language = language,
    ...
    int rc = init_template(&base_template);
